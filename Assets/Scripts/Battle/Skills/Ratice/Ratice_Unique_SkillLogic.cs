...
namespace Battle.Skills {
	class Ratice_Unique_SkillLogic : BasePassiveSkillLogic {
		public override void OnAnyCastingDamage(CastingApply castingApply, int chain){
			var skillOwner = UnitManager.Instance.GetAnUnit(passiveSkill.owner);
			if (Calculate.Distance(castingApply.Target, skillOwner) != 1 || !castingApply.Target.IsAllyTo(skillOwner)) return;
