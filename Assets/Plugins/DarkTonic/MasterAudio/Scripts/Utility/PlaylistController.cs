...
namespace DarkTonic.MasterAudio {
    ...
    public class PlaylistController : MonoBehaviour {
        ...
        public AudioSource transitioningAudio;
        ...
        private void Update() {
            ...
            if (IsCrossFading) {
                // cross-fade code
                if (activeAudio.volume >= _activeAudioEndVolume) {
                ...
                activeAudio.volume = ratioPassed * _activeAudioEndVolume;
                transitioningAudio.volume = _transitioningAudioStartVolume * (1 - ratioPassed);
                // end cross-fading code
            } 

            if (!activeAudio.loop && activeAudio.clip != null) {
				if (AudioUtil.IsAudioPaused(activeAudio)) { // 7/12/2017, changed this if before the next one (.isPlaying) because paused tracks were stopping.
				...
				if (!activeAudio.isPlaying) {
                    ...
                    if (hasNoNextSong && !activeAudio.isPlaying) { // 8/21/2017, the last song would not unload because it was never detected.
						FirePlaylistEndedEventIfAny();
						CeaseAudioSource(activeAudio); // this will release the resources
                        return;
                    }
                }
                ...
                if (ShouldNotSwitchEarly) { // keep default of shouldAdvance = false. Special logic, do not try to fade in early!
                    ...
                    var currentClipTime = activeAudio.clip.length - activeAudio.time - AudioUtil.AdjustEndLeadTimeForPitch(CrossFadeTime, activeAudio);
                    var clipFadeStartTime = AudioUtil.AdjustEndLeadTimeForPitch(AudioUtil.FrameTime * FramesEarlyToTrigger, activeAudio);
			...
			if (activeAudio.loop && activeAudio.clip != null) {
				if (activeAudio.timeSamples < _lastFrameSongPosition) {
					var songName = activeAudio.clip.name;
				...
				_lastFrameSongPosition = activeAudio.timeSamples;
			}
        ...
        public void PausePlaylist() {
            ...
            if (transitioningAudio.clip != null) {
        ...
        public bool UnpausePlaylist() {
            ...
            if (activeAudio.clip == null && _currentPlaylist != null) {
            ...
            if (!_scheduledSongOffsetByAudioSource.ContainsKey(activeAudio)) {
                activeAudio.Play();
				framesOfSongPlayed = 0;
				AudioUtil.ClipPlayed(activeAudio.clip, GameObj);
                ...
                activeAudio.Stop(); // stop the previous scheduled play

                ScheduleClipPlay(newSongStartOffset, activeAudio, true);
            }

            if (!_scheduledSongOffsetByAudioSource.ContainsKey(transitioningAudio)) {
                transitioningAudio.Play();
				AudioUtil.ClipPlayed(transitioningAudio.clip, GameObj);
            } else if (_songPauseTime.HasValue && _currentSchedSongDspStartTime.HasValue) {
                var unpauseTime = AudioSettings.dspTime;
                var timePaused = unpauseTime - _songPauseTime.Value;
                ...
                transitioningAudio.Stop(); // stop the previous scheduled play

                ScheduleClipPlay(newSongStartOffset, transitioningAudio, true);
            }
        ...
        public void FadeToVolume(float targetVolume, float fadeTime, System.Action callback = null) {
            ...
            if (_duckingMode == AudioDuckingMode.NotDucking) {
                ...
                _slowFadeStartVolume = activeAudio.volume;
            }
            ...
            if (_currentSong != null) {
                ...
                if (!IsCrossFading) {
                    ...
                    if (transitioningAudio != null) {
                        transitioningAudio.volume = newVolume;
                    }
                }
        ...
        private void PlaySong(MusicSetting setting, AudioPlayType playType) {
            ...
            if (playType != AudioPlayType.AlreadyScheduled) {
                ...
                if (activeAudio.clip == null) {
                    _audioClip = activeAudio;
                    _transClip = transitioningAudio;
                } else if (transitioningAudio.clip == null) {
                    _audioClip = transitioningAudio;
                    _transClip = activeAudio;
                } else {
                    // both are busy!
                    _audioClip = transitioningAudio;
                    _transClip = activeAudio;
        ...
        public void FinishLoadingNewSong(AudioClip clipToPlay, AudioPlayType playType) {
            ...
            if (_currentSong != null) {
                _currentSong.lastKnownTimePoint = activeAudio.timeSamples;
            ...
            if (playType == AudioPlayType.PlayNow) {
                ...
                if (firstMatchingGroupController != null) {
                    var matchingTimeSamples = firstMatchingGroupController.activeAudio.timeSamples;
                    ...
                    switch (_currentPlaylist.songTransitionType) {
                        case MasterAudio.SongFadeInPosition.SynchronizeClips:
                            if (!songTimeChanged) { // only do this for single Controllers. Otherwise the sync group code above will get defeated.
                                ...
                                if (playType == AudioPlayType.PlayNow) {
                                    ...
                                    var newTimeSamples = _lastSongPosition.HasValue ? _lastSongPosition.Value : activeAudio.timeSamples;
                                    if (transitioningAudio.clip != null && newTimeSamples >= transitioningAudio.clip.samples - 1) { // prevent the "index too far" error if you position past the end of clip.
                                        newTimeSamples = 0;
                                    }

                                    _lastSongPosition = null;

                                    transitioningAudio.timeSamples = newTimeSamples;
                                } else {
                                    if (ShouldNotSwitchEarly) {
                                        // DO NOT set _transitioningAudio.timeSamples = 0, because it will "skip"
                                    } else {
                                        transitioningAudio.timeSamples = 0;
                                    }
                                }
                            }
                            break;
                        case MasterAudio.SongFadeInPosition.NewClipFromLastKnownPosition:
                            ...
                            if (thisSongInPlaylist != null) {
                                ...
                                if (transitioningAudio.clip != null && lastKnownTimePoint >= transitioningAudio.clip.samples - 1) { // prevent the "index too far" error if you position past the end of clip.
                                    lastKnownTimePoint = 0;
                                }

                                transitioningAudio.timeSamples = lastKnownTimePoint;
                            }
                            break;
            ...
            if (clipWillBeAudibleNow) {
                activeAudio = _audioClip;
                transitioningAudio = _transClip;
        ...
        private double CalculateNextTrackStartTimeOffset() {
            ...
            return GetClipDuration(activeAudio);
        }
        ...
        private void SetDuckProperties() {
            _originalMusicVolume = activeAudio == null ? 1 : activeAudio.volume;
